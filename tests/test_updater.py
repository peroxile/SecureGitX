...
from securegitx.updater import (
    DEFAULT_SOURCE_URL,
    UpdateError,
    UpdateResult,
    _is_newer,
...
def _patch_rules_dir(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("securegitx.updater._RULES_DIR", tmp_path)
    monkeypatch.setattr("securegitx.updater._RULES_FILE", tmp_path / "rules.json")
    monkeypatch.setattr("securegitx.updater._BACKUP_FILE", tmp_path / "rules.json.bak")
    monkeypatch.setattr("securegitx.updater._META_FILE", tmp_path / "rules.json.meta")
...
def test_update_installs_newer_version(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("securegitx.updater._fetch", _mock_fetch(remote))
...
def test_update_invalid_rule_pattern_raises(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("securegitx.updater._fetch", _mock_fetch(_as_bytes(bad_data)))
...
def test_update_network_error_raises(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("securegitx.updater._fetch", _fail)
